classdef NmpcControl_overtake < handle
    ...
    methods
        function obj = NmpcControl_overtake(car, H)
            ...
            Q_track = diag([0, 1, 20, 5]);    % State tracking weights:
                                              % [x position (unused), y position, heading angle, velocity]
                                              % Higher weight (30) on velocity tracking
            R_track = diag([10, 20]);         % Input cost weights for smoother control:
                                              % [steering angle, throttle]
            Q_term = Q_track/2;               % Terminal cost weight
            
            %  Ellipsoidal constraint matrix
            % Design H matrix for collision avoidance
            % Car dimensions: 4.3m long, 1.8m wide
            % Design ellipsoidal constraint considering safety margins
            H_const = diag([1/64, 1/9]);    % Roughly corresponds to safe distance of 8m longitudinally 
                                            % and 3m laterally     
            ...
            other_vel = obj.x0other(4);  % Constant velocity of other car
            for k = 1:N
                % Predict other car position 
                other_x = obj.x0other(1) + other_vel * (k-1) * dt;
                other_y = obj.x0other(2);
                
                % Relative position vector
                p = [X(1,k) - other_x; X(2,k) - other_y];

                % Ellipsoidal constraint
                opti.subject_to(p' * H_const * p >= 1);
            end
